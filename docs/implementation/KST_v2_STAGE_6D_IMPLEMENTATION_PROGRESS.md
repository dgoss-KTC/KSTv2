...
- `pt_mstr.pt_mfg_lead`, `pt_sfty_time`, `pt_sfty_stk` are all **`decimal(28,0)`** (not integer/smallint) — mapped to nullable C# `decimal?`, not `int?`, per the contract's "do not truncate source precision" rule.
